...
all: clear-all build up composer-install frontend-build db-migrate var-preps cleanup-updates
...
.PHONY: all restart build up down composer-install composer-update db-migrate db-migration-generate db-migration-rollback
.PHONY: refresh-partitions cleanup-updates frontend-build clear-all clear-cache clear-logs
.PHONY: debug-container debug-routes debug-messenger test-new-arch
.PHONY: analyze phpstan phpstan-fix phpstan-baseline cs-check cs-fix quality
